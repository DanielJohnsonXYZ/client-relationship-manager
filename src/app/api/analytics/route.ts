...
export async function GET(request: NextRequest) {
  try {
    ...
    const avgSentiment = totalCommunications > 0 
      ? communicationsData?.reduce((acc, comm) => acc + (comm.sentiment_score || 0.5), 0) / totalCommunications 
      : 0; // No satisfaction data when no communications
    const clientSatisfaction = totalCommunications > 0 ? Math.round(avgSentiment * 100) : 0;
