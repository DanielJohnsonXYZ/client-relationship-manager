...
export async function GET(request: NextRequest) {
  try {
    ...
    const dataContext = {
      ...
      avgSentiment: (communications?.length || 0) > 0 
        ? communications?.reduce((acc, comm) => acc + (comm.sentiment_score || 0.5), 0) / communications.length
        : 0, // No sentiment data when no communications
...
function generateFallbackInsights(dataContext: any) {
  ...
  // Communication sentiment - only if there are communications
  if (dataContext.totalCommunications > 0 && dataContext.avgSentiment > 0.7) {
    insights.push({
      id: `insight-${Date.now()}-3`,
      type: 'trend',
      title: 'Positive Communication Sentiment',
      description: `${dataContext.totalCommunications} communications maintain ${(dataContext.avgSentiment * 100).toFixed(1)}% positive sentiment, indicating strong client satisfaction.`,
      impact: 'medium',
      confidence: 80,
      created_at: new Date().toISOString()
    });
  } else if (dataContext.totalCommunications === 0) {
    insights.push({
      id: `insight-${Date.now()}-no-comms`,
      type: 'opportunity',
      title: 'Start Tracking Communications',
      description: 'Begin tracking client communications to unlock insights about client satisfaction, response times, and relationship health.',
      impact: 'medium',
      confidence: 100,
      created_at: new Date().toISOString()
    });
