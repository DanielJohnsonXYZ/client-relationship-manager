-- Create only the missing tables for Loom integration
...
CREATE INDEX IF NOT EXISTS idx_external_activities_client_date ON public.external_activities(client_id, activity_date DESC);
CREATE INDEX IF NOT EXISTS idx_external_activities_integration ON public.external_activities(integration_id);
CREATE INDEX IF NOT EXISTS idx_sync_logs_integration_date ON public.integration_sync_logs(integration_id, created_at DESC);
...
DROP TRIGGER IF EXISTS sync_external_to_communication ON public.external_activities;
